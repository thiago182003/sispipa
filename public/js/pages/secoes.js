...
document.addEventListener("DOMContentLoaded", function (e) {
    ...
    dataTable = new DataTable(table, {
        ...
        columnDefs: [
            ...
            },
            {
                targets: 2,
                render: function (data, type, row, meta) {
                    return '<span class="text-heading">' +  (row.superior ? row.superior.sigla : "") + "</span>"
                }
            },
            {
                targets: 3,
                render: function (data, type, row, meta) {
                    return '<span class="text-heading">' +  (row.oms ? row.oms.sigla : "") + "</span>"
                }
            },
            {
                targets: -1,
            ...
            topEnd: {
                features: [{
                    ...
                    buttons: [{
                        ...
                        text: '<i class="icon-base bx bx-plus icon-sm me-0 me-sm-2"></i><span class="d-none d-sm-inline-block">Adicionar Seção</span>',
    ...
    FormValidation.formValidation(document.getElementById('form-secao'), {
        ...
        $.post('/administrativo/secoes/store', {
            id: id.value,
            nome: nome.value,
            sigla: sigla.value,
            om_id: selectoms.value,
            superior_id: selectsecoes.value
        }).done(function (response) {
            Swal.fire({
                icon: "success",
                title: "Inserido",
                text: "Seção inserida com sucesso!",
            ...
            Swal.fire({
                ...
                text: "Ocorreu um erro ao tentar inserir a Seção.",
...
function editar(id){
    $.ajax({
        url: '/administrativo/secoes/show',
        method: 'GET',
        data: {
            id: id,
        },
        success: function (response) {
            ...
            $("#selectoms").val(response.om_id);
            document.getElementById("selectsecoes").addEventListener("change", function() {
                
                // Aqui você pode chamar outra função ou executar qualquer ação
            });
            $("#selectoms").change();
            $("#selectsecoes").val(response.superior_id);
            $("#selectsecoes").change();
...
function confirmarExclusao(id) {
    ...
    Swal.fire({
        ...
        if (resultado.isConfirmed) {    
            ...
            $.post('/administrativo/secoes/delete', { id: id })
                .done((resposta) => {
                    Swal.fire({
                        icon: "success",
                        title: "Deletado",
...
$('#selectoms').change(function () {
    var omId = $(this).val();
    var secaoSelect = $('#selectsecoes');
    
    secaoSelect.html('<option value="">Carregando...</option>');

    if (omId) {
        $.get('/administrativo/oms/' + omId + '/secoes', function (data) {
            secaoSelect.empty();
            if (data.length > 0) {
                secaoSelect.append('<option value="">Selecione...</option>');
                $.each(data, function (index, secao) {
                    secaoSelect.append('<option value="' + secao.id + '">' + secao.nome + '</option>');
                });
            } else {
                secaoSelect.append('<option value="">Nenhuma seção encontrada</option>');
            }
        }).fail(function () {
            secaoSelect.html('<option value="">Erro ao carregar seções</option>');
        });
    } else {
        secaoSelect.html('<option value="">Selecione uma OM primeiro...</option>');
    }
});